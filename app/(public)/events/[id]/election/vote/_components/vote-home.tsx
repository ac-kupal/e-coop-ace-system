...
const VoteHome = ({ eventId }: Props) => {
    ...
    const { election, isLoading } = useElectionWithEventAndPositionsAndCandidates(eventId)
    ...
    if (election.status !== "live") return <InvalidPrompt message="Election is not live" />;

    if (election.positions.length === 0) return (<InvalidPrompt message="It seems like this election doesn't have positions and candidates yet. Please contact admin" />);
